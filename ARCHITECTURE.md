...
| 2 | **Cross-platform** | `cross_platform_dedup()` — fuzzy matching across *different platforms only*. Company token overlap ≥ 0.5 (min-set denominator), title Jaccard similarity ≥ 0.6, location match (with city aliases: München/Munich, Köln/Cologne). Keeps higher-priority platform (LinkedIn > Xing > Stepstone > Indeed). | Same job reposted across LinkedIn/Xing/Stepstone/Indeed with company name variants |
| 3 | **Cross-run** | `load_previous_run_keys()` — compares against the immediate previous run using URL keys (all history) + title keys (previous run only). | Consecutive-day duplicates from 24h window overlap, LinkedIn re-lists |
...
| `fetch_startupjobs_jobs()` | `cloudscraper` HTML, `data-post-template-target` attrs. Delegates parsing to `_parse_startupjobs_page()` |
...
| `fetch_glassdoor_jobs()` | `cloudscraper` (chrome), JSON-LD ItemList, `_KE` company extraction, `ageInDays` filtering. Delegates to `_parse_glassdoor_item()`, `_glassdoor_fetch_with_retry()`, `_extract_age_lookup()` |
...
| `cross_platform_dedup()` | Fuzzy cross-platform dedup: company overlap ≥ 0.5 + title Jaccard ≥ 0.6 + location match |
| `load_previous_run_keys()` | URL + title keys from previous run for cross-run dedup |
...
| `verify_startupjobs()` / `verify_glassdoor()` | Pre-fetched description or cloudscraper fallback |
